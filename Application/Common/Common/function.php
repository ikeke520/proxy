...
function is_login()
{
    ...
    $user=$user1= session('login_userid');
...
function checkSign($param="",$sign="",$key="secret",$type="MD5"){
	$str=toString($param);
	if($type($str.$key)==$sign){
		return true;
	}else{
		return false;
	}
}
function getSignstr($param,$key='o5@Dpje@03VeaFyZGgc996kzTn%@MSct',$type="MD5"){
	ksort($param);
	$tmpStr = '';
	foreach($param as $k=>$v){
		$tmpStr = $tmpStr.$k.'='.$v.'&';
