...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function is_weixin(){
		...
		echo "请在微信浏览器中访问";exit;
		return false;
	}
	/**
	 * 微信接口实例并存储更新微信用户
	 */
	public  function wxconstruct(){
		if(!$this->is_weixin()){
			return false;
		}
		...
		if(!session('user_token')){
			...
			if(!$_REQUEST['code']){
				//第一次进来获取code
				$url=$this->curPageURL();
				$curl=$this->getOauthRedirect($url);
				header("location:".$curl);
	
			}
			//如果由refresh_token，刷新
			if(session('refresh_token')){
				$ret=$this->getOauthRefreshToken();
			}else{
				//获取openid,access_token
				$ret=$this->getOauthAccessToken();
			}
			//保存cookie
			if($ret){
				session('user_token',$this->user_token,$this->expires_in);
				session('openid',$this->openid,$this->expires_in);
				session('expires_in',$this->expires_in,$this->expires_in);
				session('refresh_token',$this->refresh_token,30*24*3600);
			}
		}else{
			//判断user_token是否失效
			$r=$this->getOauthAuth(session('user_token'),session('openid'));
		...
		//}else{
		$wxuser=$this->getOauthUserinfo();
		$swxuser=$this->getUserInfo($wxuser['openid']);
		$wxuser['subscribe']=$swxuser['subscribe'];
		//session('wxuser',$wxuser,7200);
		//获取微信js_api
		//}
		//存储用户
		/* $ret=M('wechatmember')->field('id')->where('openid="'.$wxuser['openid'].'"')->find();
		 if(!$ret){
		 M('wechatmember')->add($wxuser);
		 }else{
		 M('wechatmember')->where('id="'.$ret['id'].'"')->save($wxuser);
		} */
		return $wxuser;
	}
}
