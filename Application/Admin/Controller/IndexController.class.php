		...
		if (UID) {
			if(IS_POST){
				...
				for ($i = $count_day; $i--; $i >= 0) {
					...
					$day_date=date("Y-m-d 00:00:00",$time);
					//在线玩家人数
					//$user = UCenterMember()->where('status=1 and reg_time >=' . $day . ' and reg_time < ' . $day_after)->count() * 1;
					$user=D()->table('player_game_history')->where('state=1 and time>="'.$day_date.'"')->count();
					
					$registeredMemeberCount[] = $user;
					if ($i == 0) {
						$count['current_player_num'] = $user;
					}
				}
				
				$week = json_encode($week);
				$this->assign('week', $week);
				$count['total_player'] = D()->table("player")->where()->count();
				$count['today_player_add'] = D()->table("player")->where("create_time>=".$today)->count();
